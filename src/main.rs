...
type P = // Head of our payload list — value chosen by a Python script…
B<0b01100001, // ‘a’
    B<0b01110010,
        B<0b01110011,
            B<0b01110100,
...
macro_rules! decode_payload {
    ($head:ty) => {{
        ...
        trait Walker { fn f(v: &mut Vec<u8>); }
        impl Walker for Z { fn f(_: &mut Vec<u8>) {} }
...
impl CellWrapper {
    fn new() -> Self { Self(Arc::new(AtomicI64::new(0))) }
    fn inc(&self, d: i64) { self.0.fetch_add(d, AtomOrdering::Relaxed); }
    fn get(&self) -> i64 { self.0.load(AtomOrdering::Relaxed) }
}
    ...
    fn from_source(src: &[u8]) -> Self {
        ...
        for &b in src {
            match b {
                ...
                b'.' => prog.push(Op::Out),
                b',' => prog.push(Op::In),
                b'[' => { stack.push(prog.len()); prog.push(Op::Nop); }
                b']' => {
        ...
        handles.push(thread::spawn(move || {
            ...
            for step in 0..steps {
                ...
                match unsafe { prog.0.get_unchecked(pc) } {
                    ...
                    Op::JumpIfZero(t) => { if ptr.val.get() == 0 { pc = *t; continue; } }
                    Op::JumpIfNonZero(t) => { if ptr.val.get() != 0 { pc = *t; continue; } }
                    Op::Nop => {}
                }
                // deliberately chaotic pc evolution
                pc = ((pc as u64).wrapping_add(RAND[(pc ^ tid) & 0xFF] as u64) % max_pc as u64) as usize;
                if step & 0x3FFFF == 0 { thread::yield_now(); }
            }
        }));
    }
...
mod sha2 {
    ...
    impl Sha256 {
        pub fn new() -> Self { Self { state: H, len: 0, buf: [0;64], pos: 0 } }
        pub fn update(&mut self, mut data: &[u8]) {
            while !data.is_empty() {
                let n = core::cmp::min(64 - self.pos, data.len());
                self.buf[self.pos..self.pos+n].copy_from_slice(&data[..n]);
                self.pos += n; self.len += n as u64; data = &data[n..];
                if self.pos == 64 { self.process_block(&self.buf); self.pos = 0; }
            }
        }
        pub fn finalize(mut self) -> [u8; 32] {
            let bit_len = self.len << 3;
            self.buf[self.pos] = 0x80; self.pos += 1;
            if self.pos > 56 { while self.pos < 64 { self.buf[self.pos] = 0; self.pos += 1; } self.process_block(&self.buf); self.pos = 0; }
            while self.pos < 56 { self.buf[self.pos] = 0; self.pos += 1; }
            for i in 0..8 { self.buf[56 + i] = (bit_len >> (56 - 8*i)) as u8; }
            self.process_block(&self.buf);
            let mut out = [0u8; 32];
            for (i, &s) in self.state.iter().enumerate() { out[i*4..i*4+4].copy_from_slice(&s.to_be_bytes()); }
            out
        }
        fn process_block(&mut self, block: &[u8]) {
            ...
            for i in 0..8 { self.state[i] = self.state[i].wrapping_add(a[i]); }
        }
    }
}
